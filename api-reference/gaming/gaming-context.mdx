...
`GamingContext` represents a game instance—either a solo game or a multiplayer session. The SDK maintains a `current` singleton that reflects the active context. You do not create a `GamingContext` directly in most cases; instead, the SDK sets `GamingContext.current` when the user enters a context via a context dialog (such as `ContextSwitchDialog` or `CreateContextDialog`).
...
- **`ContextSwitchDialog`** — switches to an existing context selected by the user.
