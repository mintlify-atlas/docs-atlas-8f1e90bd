...
Call `startWithCompletion(_:)` to fire the request. The completion closure receives a connection, a result, and an error.
...
`startWithCompletion(_:)` returns an `id<FBSDKGraphRequestConnecting>` value you can use to cancel the request before it completes.
